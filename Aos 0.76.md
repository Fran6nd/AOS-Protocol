...
- **World Update** carries only active player slots and prefixes each entry with its player ID, instead of a fixed 32-slot array.
...
| [World Update](#world-update) | S→C | in-game | `id` u8, then N× (`pid` u8, `pos` f32×3, `ori` f32×3) LE | 0.75 *(reformatted in 0.76)* | Periodic broadcast of active player slots' position + orientation. | Variable-length list; one entry per currently-occupied slot. Each entry is 25 bytes. |
...
This ID also identifies `Map Cached` in 0.76. Disambiguation is by phase — `Handshake Init` precedes `Map Start`; `Map Cached` only ever follows it.
