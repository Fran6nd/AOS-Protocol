...
Wire-format reference for AoS network protocol version 0.76. Transport is ENet over UDP. All multi-byte integers and floats are little-endian unless otherwise noted. Strings are CP437-encoded.

0.76 is a small superset of [0.75](Aos%200.75.md). The differences are:

- **Packet 2 (World Update)** carries only active player slots and prefixes each entry with its player ID, instead of a fixed 32-slot array.
- **Packet 18 (Map Start)** is extended with a CRC32 of the uncompressed map and a map name, allowing client-side caching.
- **Packet 31 (Map Cached)** is new — the client tells the server whether the map (identified by CRC32 from `Map Start`) is already cached locally, so the server can skip retransmitting `Map Chunk` frames.

All other packets are byte-for-byte identical to 0.75 and the `Introduced` column reflects each packet's first-appearance version.
...
| `map-transfer` | Server pushes the compressed map to the client (skipped when the client confirms a cache hit via `Map Cached`). Bracketed by `Map Start` and the final `State Data`. |
...
| [2. World Update](#2-world-update) | S→C | in-game | `id` u8, then N× (`pid` u8, `pos` f32×3, `ori` f32×3) LE | 0.75 *(reformatted in 0.76)* | Periodic broadcast of active player slots' position + orientation. | Variable-length list; one entry per currently-occupied slot. Each entry is 25 bytes. |
...
| [18. Map Start](#18-map-start) | S→C | map-transfer | `id` u8, `map_size` u32 LE, `crc32` u32 LE, `map_name` cp437 (NUL-terminated) | 0.75 *(extended in 0.76)* | Server begins sending the map after ENet connect or on map advance. | `map_size` is the byte length of the upcoming compressed map stream. `crc32` lets the client check whether the map is already cached locally. |
| [19. Map Chunk](#19-map-chunk) | S→C | map-transfer | `id` u8, `data` bytes (DEFLATE/zlib map payload) | 0.75 | Repeated after `Map Start` until the entire compressed map has been transferred; the next packet is `State Data`. Skipped entirely when the client replies with `Map Cached`. | Concatenate all chunk payloads in order, then zlib-decompress to obtain the AOS-format map. |
...
| [31. Map Cached](#31-map-cached) | C→S | map-transfer | `id` u8, `cached` u8 | 0.76 | Sent by the client right after receiving `Map Start`, to tell the server whether the map (matched by `crc32`) is already cached locally. | `cached`: 0 = not cached (server proceeds with `Map Chunk` stream), 1 = cached (server may skip directly to `State Data`). |
...
| 9 | `z` | f32 LE | world Z, vertical axis (downward positive in 0.76 maps; ~0 at the sky, ~63 at solid ground) |
...
- **ID:** `0x02` · **Direction:** S→C · **Phase:** in-game · **Introduced:** 0.75 *(reformatted in 0.76)* · **Size:** 1 + N × 25 bytes
...
This is the main difference from 0.75, which uses a fixed 32-slot array without per-entry player IDs.
...
- **Trigger:** Sent once after the last `Map Chunk` (or directly after `Map Cached` confirms a cache hit) to signal that the join is complete and to deliver gamemode parameters.
...
- **ID:** `0x12` · **Direction:** S→C · **Phase:** map-transfer · **Introduced:** 0.75 *(extended in 0.76)* · **Size:** variable (≥ 10 bytes)
...
- **Trigger:** Repeated zero or more times after `Map Start` until the full compressed map has been delivered. The next packet after the final chunk is `State Data`. When the client replied to `Map Start` with `Map Cached = 1`, the server skips this stream entirely and goes directly from `Map Start` to `State Data`.
